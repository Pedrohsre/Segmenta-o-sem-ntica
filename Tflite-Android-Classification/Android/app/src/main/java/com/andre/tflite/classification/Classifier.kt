...
data class SegmentationResult(
    val segmentationMask: Array<Array<Int>>,
    val coloredMask: Bitmap,
...
class Classifier(
    ...
    // Cores para visualização da segmentação
    private val segmentationColors = arrayOf(
        Color.RED,     // Classe 0 - Pet
        Color.GREEN,   // Classe 1 - Fundo
        Color.BLUE     // Classe 2 - Borda
    )
    ...
    fun segment(bitmap: Bitmap): SegmentationResult {
        ...
        // Para segmentação, o output é [1, height, width, num_classes]
        val output = Array(1) { Array(imageSize) { Array(imageSize) { FloatArray(numClasses) } } }
        interpreter.run(input, output)

        // Converter probabilidades para classes
        val segmentationMask = Array(imageSize) { Array(imageSize) { 0 } }
        var totalConfidence = 0f
        
        for (i in 0 until imageSize) {
            for (j in 0 until imageSize) {
                val probabilities = output[0][i][j]
                var maxProb = 0f
                var maxClass = 0
                ...
                segmentationMask[i][j] = maxClass
                totalConfidence += maxProb
            }
        }
        
        val averageConfidence = totalConfidence / (imageSize * imageSize)
        
        // Criar bitmap colorido da máscara
        val coloredMask = createColoredMask(segmentationMask)
        
        return SegmentationResult(segmentationMask, coloredMask, averageConfidence)
    }

    private fun createColoredMask(mask: Array<Array<Int>>): Bitmap {
        val coloredBitmap = Bitmap.createBitmap(imageSize, imageSize, Bitmap.Config.ARGB_8888)
        
        for (i in 0 until imageSize) {
            for (j in 0 until imageSize) {
                val classIndex = mask[i][j]
                val color = if (classIndex < segmentationColors.size) {
                    segmentationColors[classIndex]
                } else {
                    Color.BLACK
                }
                coloredBitmap.setPixel(j, i, color)
            }
        }
